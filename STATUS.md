...
## ДДС по входяща доставка — 2026-08-05 (PR #94, ЖИВО)

Фактурата на доставчика е с ДДС, а доставката се въвежда по единични
цени и нямаше НИКАКВО понятие за ДДС (разписката: само Междинна /
Отстъпка / Общо). Оттам сумите не се връзваха с фактурата.

Бутон **„ДДС"** в модала на ЧАКАЩА доставка + поле за ставка:

- **към сумите** — цените остават без ДДС; разписката добавя
  Данъчна основа / ДДС X% / Общо с ДДС. Себестойността остава чиста.
- **към цените** — умножава единичните цени по (1+r/100). Тази цена
  става покупна за продукта → влиза в себестойността.
- **премахни** — обратимо; ако е било вградено, дели цените обратно.

⚠️ `prices_include_vat` е предпазителят срещу двойно прилагане (иначе
×1.44). Второ „към цените" връща 409.

⚠️ Ставката е ПОЛЕ, не константа — при гръцки доставчик (ВОП)
фактурата често е с 0%.

Само за pending: след потвърждаване цените вече са в склада →
корекцията е през редакция или кредитно известие.

Мигр. 104 (vat_rate, prices_include_vat) — ПРИЛОЖЕНА НА ПРОД ръчно.
Бекъп преди това: `~/gqf-backups/gqf-prod-pre-vat-20260805-1111.sql.gz`.
