...
describe("allocateFefo", () => {
  ...
  it("при само изтекла наличност грешката казва колко е изтекло и до кога", async () => {
    // Прод случай (продукт 59, мляко CHOCOFULL): 12 бр в наличност, но с
    // изтекъл срок → „налични 0" объркваше склада (изглежда като липсваща
    // стока). Грешката вече обяснява: има стока, но е изтекла.
    const client = mkClient([
      {
        batch_id: 4,
        batch_number: "АВТО-2-59",
        expiry_date: "2026-06-14", // изтекла спрямо TODAY=2026-06-22
        purchase_price: "1.00",
        available: "12",
      },
    ]);
    await expect(
      allocateFefo(client, 59, 1, 12, { today: TODAY }),
    ).rejects.toThrow(
      "Недостатъчна неизтекла наличност за продукт 59: искани 12, налични 0 (12 бр. са с изтекъл срок, последен 2026-06-14)",
    );
  });
